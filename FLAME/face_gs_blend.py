...
from .transform_network import TransformerModel
import logging
...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.embedding = TransformerModel(input_dim=51, model_dim = 128, n_heads=4, n_layers = 1, output_dim = 51)
    ...
    def sample_from_face(self, v, f, N = 6):
        t = np.arange(f.shape[0])
        f_new = np.repeat(t, N, axis=0)
        
        random_vector = torch.randn( N * f.shape[0], 3)
        magnitude = torch.norm(random_vector, dim = 1, keepdim= True)
        barycentric_coord = random_vector / magnitude
        return f_new, barycentric_coord.numpy()

    def sample_from_verticle(self, v, f, N = 6):
        t = np.arange(v.shape[0])
        f_new = np.repeat(t, N, axis=0)
        
        random_vector = torch.randn( N * v.shape[0], 3)
        magnitude = torch.norm(random_vector, dim = 1, keepdim= True)
        barycentric_coord = random_vector / magnitude
        return f_new, barycentric_coord.numpy()
    ...
    def blendshape_ICT(self, params):  
        batch_size = params.shape[0]
        
        vertices = self._xyz.unsqueeze(dim=0).expand([batch_size, -1, -1]) + (self._xyz_b * params).sum(-1)
        
        self._mf = params #.blend[0,1:]
        
        processed_rotation = self._rotation
        #processed_rotation = self._rotation + (self._rotation_b * params).sum(-1)
        
        #processed_features = self._features_dc + (self._features_b * params.blend[:, 1:]).sum(-1)
        processed_features = None
        
        return vertices, processed_rotation, processed_features
    ...
    def prepare_xyz(self, params , args):
        ...
        #self.processed_features = self.rgb_activation(_features)
        
        #blend = params.blend[:, 1:]
        # from IPython import embed
        # embed()
        #blend = self.embedding(params.blend[:,1:], params.blend[:, 1:])
        blend = params.blend[:,1:]
        
        vertices, processed_rotation, processed_features= self.blendshape_ICT(blend)
        #vertices, T_diff, processed_rotation, processed_features = self.run_blendshape(
        #     self._xyz, params
        #)

        self.processed_xyz = vertices[0]                       #(n, 3)  
        self.processed_rotation = processed_rotation           #(n, 4)
        #self.processed_features = processed_features           #(n, 16, 3)

        # _scaling = self._scaling
        # _scaling_b = self._scaling_b
        # self._scaling = _scaling + torch.sum(_scaling_b * self._mf, dim=-1)
        
        # _opacity = self._opacity
        # _opacity_b = self._opacity_b
        # self._opacity = _opacity + torch.sum(_opacity_b * self._mf, dim=-1)
    ...
    # change xyz, rotation, opacity, feature
    # xyz: 0.0054, 31.47,  0.0074, 29.57 ,
    # xyz, rotaion: 0.0043, 34.3， 0、0074， 29.8          
    # xyz, feature: 
    # xyz, feature, opacity, scale: blend(feature)
    # blend(feature): 拟合变慢了
    # 过拟合现象明显
    # 只优化xyz， opacity，不容易过拟合，0.0074是个坎
    # the different images with pixel also influence the PSNR metric. 
    
    @property
    def get_scaling(self):
        # _scaling = self._scaling
        _scaling = self._scaling
        # _scaling = _scaling + torch.sum(self._scaling_b * self._mf, dim=-1)
    ...
    def get_rotation(self):
        return self.rotation_activation(self.processed_rotation)
    ...
    def create_from_face(self, params, args, spatial_lr_scale: float, replace_v = False):
        ...
        if replace_v:
            ...
            v = canonical_verts.cpu().numpy()               #(5023, 3) 
            print("Origin Points", v.shape)
        
        f = faces[1].verts_idx.cpu().numpy()                 #(9976, 3) FLAMe: point, face
        #face_id, barycentric_coord = self.sample_possion_points(v, f, args.init_face_point_number)           #71808, possion points
        
        face_id, barycentric_coord = self.sample_from_face(v, f, N = 3 * 3)
        ...
        # bind_v_id, barycentric_coord = self.sample_from_verticle(v, f, N = 3 * 3)
        
        # self.bind_v_id = torch.from_numpy(bind_v_id).to(args.data_device)
        # self.barycentric_coord = torch.from_numpy(barycentric_coord).to(args.data_device)                     #71460
        
        # 71092, 
        # 9976 + 2934 = 11680, f, v: 6036 
        # face_id > 9976 = 2934
        # 
        # logger.info("embed")
        # from IPython import embed
        ...
        print("Face num: ", face_id.shape[0])
        
        self.n_pose_params = 0
        self.n_expr_params = 51 #51 %faceunity
        self.n_total_params = self.n_expr_params + self.n_pose_params

        with torch.no_grad():

            vertices = canonical_verts.unsqueeze(dim = 0).cuda()
            
            # # related to the canonical mesh
            ...
            albedos = (torch.rand(point_vertices.shape[0],3)/255.).float().cuda() #color
            ...
            #features_b = torch.zeros((fused_color.shape[0], 1, 3, self.n_total_params)).float().cuda()
        ...
        # _xyz: (71808, 3)
        # _xyz_b:(71808, 3, 50)
        # _features_b: (71808, 16, 3, 50)

        #flametex = None
        
        # print(self.face_id)
        # print(args.blend_template_path)
        # self.loadData(load_file=args.blend_template_path)
        
        # exp_face:(5023, 3 ,51) : 0, 1, 5022 
        # canonical:(5023, 3)
    ...
    #     self.canonical = nn.Parameter(to_tensor(data['canonical']), requires_grad=False)
    #     self.exp_face = nn.Parameter(to_tensor(data['exp_face']), requires_grad=False)
        
    #     self.blend_n_face = 5023 * 9
    #     self.blend_face_id = self.faces[self.face_id[]]
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._xyz_b.shape[2] + 1):
            l.append('x_{}'.format(i))
            l.append('y_{}'.format(i))
            l.append('z_{}'.format(i))
        return l

    def save_ply(self, path):
        mkdir_p(os.path.dirname(path))

        xyz = self.processed_xyz.detach().cpu().numpy()
        xyz_0 = self._xyz.detach().cpu().numpy()
        
        normals = np.zeros_like(xyz)
        #features_dc, features_rest = self.processed_features[:,0:1,:],  self.processed_features[:,1:,:]
        features_dc, features_rest = self._features_dc, self._features_rest       #??? some bug with propressed_roatation may be resulted by contiguous() 
        ...
        rotation = self.processed_rotation.detach().cpu().numpy()
        ...
        #(N,3,51) -> (N, 51, 3) -> N, 153
        postion_b = self._xyz_b.detach().transpose(1, 2).cpu().reshape(-1, 153).numpy()
        
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]
        
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, xyz_0, postion_b), axis=1)
